...
class Calibration_Curves():
    ...
    def dose_current(self, Directory=False, PdfPages=False, stdev=0.06, depth=[0], colors=[0], table=True,Voltages=["40kV"]):
        ''', 
...
#         '''
    ...
    def opening_angle(self, Directory=False, Unknown_diameter=np.linspace(3, 10, 20), PdfPages=False, tests=["without_Al_Filter"]):
        ...
        for j in range(len(tests)):
            ...
            ax2.plot(h, self.linear(h, *popt), '-', lw=1, label=line_fit_legend_entry, markersize=9)
...
if __name__ == '__main__':
    ...
    #scan.dose_current(stdev=0.04, PdfPages=PdfPages, Directory=Directory, depth=depth, colors=colors, table=True,Voltages=Voltages)
    #scan.dose_drop(stdev=0.04, PdfPages=PdfPages, Directory=Directory,depth=depth,colors=colors)
    scan.dose_voltage(PdfPages=PdfPages, Directory=Directory, test="without_Al_Filter", kafe_Fit=False, table=False)
