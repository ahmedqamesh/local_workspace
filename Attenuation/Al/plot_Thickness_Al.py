...
import numpy as np
'''
THis will calculate the attenuation due to a specific thickness (xray_Al_filter) of Al
'''
Attenuation_Al = [2.263E+03,2.621E+01,3.442E+00,1.128E+00, 5.684E-01,3.681E-01]#,2.778E-01,2.018E-01,1.704E-01]
Energy_range = ['2 Kev','10 Kev','20 Kev','30 Kev','40 Kev','50 Kev']#,'60 Kev','80 Kev', '100 Kev']
Al_thickness = 2.8 #Micro
xray_Al_filter =  0.015 # cm
Al_rho = 2.70 #g/cm3
x2=np.arange(0, 10, 0.00001) #cm
...
ax.annotate("%5.3f $\mu$ m"%xray_Al_filter, xy=(xray_Al_filter, 0), xytext=(xray_Al_filter+0.1, 0.2),
...
plt.tight_layout()
plt.savefig(r'Thickness_Al_xray_filter_0.015.png', bbox_inches='tight')
plt.show()
