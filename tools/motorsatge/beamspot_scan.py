...
#     dut["ms"]._write_command("FE2", address=3)
 # Search starts in negative direction 
    dut["ms"]._write_command("FE0,RP", address=2)  #(To start from the extreme left)
    dut["ms"]._write_command("FE1,RP", address=3)  # (To start from the extreme in)
    
def beamspot(size_x=60000, z=49, x_Delay=3, x=0,size_z=60000,
    ...
    def Move(step_z=False,size_x=size_x,size_z=size_z):
        ...
        for step_x in xrange(a,b,c):
            dut["ms"].read_write("MR%d" % (size_x), address=3) # x 50000,100,50 = 4.5 cm
            time.sleep(x_Delay)
            if Sourcemeter:
        ...
        dut["ms"].read_write("MR%d" % (-size_z), address=2)  # x# x 50000,100,50 = 4.5 cm 
