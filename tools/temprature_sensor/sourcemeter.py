...
class sourcemeter(HardwareLayer):
    ...
    def IV_test(self, CurrentLimit=1.000000E-01, start_V=False, step_V=False, end_V=False,
                Itterations=False, stat_delay=False, Plot=False, diode_id=True):
        ...
        File = tb.open_file('/home/silab62/MasterWork/' + str(diode_id) + ".h5", 'w')
        ...
        df.to_csv(Directory+ str(diode_id) + ".CSV", index=True)
        if Plot:
            self.Plotting_IVcurve_Stat(Directory=Directory, h5=True, Multiple=True)
